...
deny_list_detailed as (
    ...
    from (
        select 
            ...
            PREV_TAPID,
            PREV_TAPSTATUSID,
            PREV_TRANSACTIONDTM+10/24 as PREV_TRANSACTIONDTM, 
            SUBSTR((TRANSACTIONDTM - PREV_TRANSACTIONDTM),1,30) as PREV_DIFF, 
            PREV_STOPID,
            TAPSTATUSID, 
            TRANSACTIONDTM+10/24 as TRANSACTIONDTM,
            NEXT_TAPID,
            NEXT_TAPSTATUSID,
            NEXT_TRANSACTIONDTM+10/24 as NEXT_TRANSACTIONDTM,
            NEXT_STOPID
        from (
            select
                t1.TAPID,
...
data as (
    ...
    where 
        ...
        AND ENTRYTRANSACTIONDTM +10/24 <= sysdate
        AND VOIDFLAG = '0'
),
getRRN as (
        select
        ...
        where je.TRANSACTIONDTM +10/24 >= (SELECT * FROM StartDate) -0 -- adjust data set left barrier here
            AND je.TRANSACTIONDTM +10/24 <= sysdate
...
weekly_total as (
    SELECT 
        ...
        tr.ENTRYTRANSACTIONDTM +10/24 as day,        
        tr.TOKENID, 
        tr.TRANSITACCOUNTID,
        (case when ram.apportionedoperatorid = '44' then '44 - Sydney Trains'
            when  ram.apportionedoperatorid = '45' then '45 - NSW Trainlink'
    ...
    left join emv.ETS_BUS_OPERATOR_DETAIL_V bso on (bso.ETSSUBSYSTEMID = rvt.ETSSUBSYSTEMID and bso.ETSBUSOPERATORID = rvt.ETSBUSOPERATORID)
    where tr.ENTRYTRANSACTIONDTM +10/24 < trunc(sysdate,'DD') + 1 -0 - (select * from dow) +4/24 -- adjust weekly barrier here
    and FAREDUE != 0
),
daily_total as (
    SELECT 
