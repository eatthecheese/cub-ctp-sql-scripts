...
WITH MANLY_TRIPS_COMPLETE as -- GET ALL VALID MANLY FERRY TRIPS
(SELECT 
    ...
    WHERE         
        ...
        and tr.ENTRY_TRANSACTION_DTM + &l_timezoneModifier/24  between &l_testDate + 4/24 and &l_testDate + 7 + 4/24
        and tr.JOURNEY_ID <> -1
),
MANLY_JOURNEYS_COMPLETE as -- GET ALL TRIPS IN THE SAME SEGMENT AS THE MANLY FERRY TRIP
...
(select 
    dmjc.JOURNEY_ID,
    dmjc.SEGMENT_ID,
    count(dmjc.TRIP_ID) as NUM_TRIP_LEGS,
...
(select mjt.JOURNEY_ID,
    ...
    (CASE WHEN tr_start.ENTRY_STOP_ID in ('936', '939') THEN
        (CASE WHEN tr_final.EXIT_STOP_ID in ('939', '936') then 'Y'
        ELSE 'N'
        END)
    ELSE 'N'
    END) as IS_MANLY_INTRA
...
and IS_MANLY_INTRA <> 'Y'
and UNDERCHARGE_AMT <> 0
