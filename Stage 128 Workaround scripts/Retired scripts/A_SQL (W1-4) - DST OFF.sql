...
deny_list_detailed as (
    ...
    from (
        select 
            ...
            prev_tap_id,
            prev_tap_status_id, 
            prev_transaction_dtm+10/24 as prev_transaction_dtm, 
            SUBSTR((transaction_dtm-prev_transaction_dtm),1,30) as prevdiff, 
            prev_stop_id,
            tap_status_id, 
            transaction_dtm+10/24 as transaction_dtm,
            next_tap_id,
            next_tap_status_id,
            next_transaction_dtm+10/24 as next_transaction_dtm,
            next_stop_id
        from (
            select
                t1.tap_id,
            ...
            where transaction_dtm +10/24 >= trunc(sysdate-1, 'dd')+4/24  --replaced 20181206 based on email from Michael Tao
			AND transaction_dtm +10/24 < trunc(sysdate, 'dd')+4/24  --added 20181206 based on email from Michael Tao
...
data as (
    ...
    from abp_main.trip
    where 
        entry_transaction_dtm +10/24 >= (SELECT * FROM StartDate) +4/24 -0 --adjust data set left barrier here
...
getRRN as (
        ...
        where je.transaction_dtm +10/24 >= (SELECT * FROM StartDate) -0 -- adjust data set left barrier here
            AND je.transaction_dtm +10/24 <= sysdate
...
weekly_total as (
    ...
    FROM data
    where data.entry_transaction_dtm +10/24 < trunc(sysdate,'DD') + 1 -0 - (select * from dow) +4/24 -- adjust weekly barrier here
    and fare_due != 0
),
daily_total as (
    SELECT 
